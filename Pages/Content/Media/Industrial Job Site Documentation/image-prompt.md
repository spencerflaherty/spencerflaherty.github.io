# Image Prompt: Industrial Job Site Documentation
...
Captured photo and video documentation across high-profile electrical projects to fuel social media content and ad creative while doubling as sales collateral for proposals.

**The Stack:**
- Camera: Sony Mirrorless
- Stabilization: Gimbal
- Audio: Wireless Lav/Receiver
...
LEFT: An industrial building/factory silhouette (large rectangle with a triangular roof and 2-3 small square windows). In front of it, construction elements — a hard hat (simple dome shape) and two crossed tools forming an X (a wrench and a screwdriver, both simple line shapes). Slightly dithered. Represents: active industrial job site.

CENTER: A camera on a gimbal stabilizer. The camera is a small rectangle with a circular lens. The gimbal is represented by a handle (vertical rectangle) connected to the camera with a simple perpendicular line, suggesting the stabilizing mount. Solid fill. Represents: professional documentation equipment in action.

RIGHT: A smartphone or tablet shape (tall rectangle with thin border). Inside the screen, show 3-4 small image thumbnails arranged in a grid (tiny squares). Around the device, 2-3 small heart or thumbs-up shapes floating nearby (simple geometric symbols). Represents: content published on social media generating engagement.
