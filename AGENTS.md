...
- Main page template: `src/pages.njk` — emits `<link rel="canonical">` from `page.seo.url`, plus `<meta name="author">` and `<meta name="generator">`. Terminal screen is a `<main aria-label="Terminal output">` landmark; `#mobile-input` carries `aria-label="Terminal command"` and `inputmode="search"` so it supports both numeric navigation and typed commands. Page-specific data (segments, dropdown bodies, nav links, search index, input prompt, type speed, initial delay) is rendered inline as `window.__TERMINAL_DATA__`; all terminal logic lives in `src/static/js/terminal.js`, loaded via `<script defer>` so browsers cache it across pages. Theme toggle, typing animation, `grep <term>` search, and `prefers-reduced-motion` short-circuit all run inside `terminal.js`
...
- Content source: `src/content/pages/*.yaml`; current portfolio entries include SQL-driven customer reactivation, automated EV feasibility reporting, custom internal tools, AI content operations, an SEO strategy dashboard, geospatial prospecting, and a multi-channel B2B video campaign system
...
- Project deep-linking and search: each project under a `section` gets a slug (auto-derived from its title via `slugify`, or overridden by an optional `slug` field in YAML / Keystatic). The dropdown renders with that `id`, and `terminal.js` auto-opens the matching dropdown on `location.hash` load and `hashchange`. `.eleventy.js` also builds a site-wide project search index from each entry's title, section heading, description, stack, media alt/title/caption, button labels, note, and related labels. In the terminal, `grep <term>` searches that index: one match redirects to the anchored dropdown, multiple matches show numbered choices, and no matches returns a terminal-style hint. Each project also accepts an optional `related` array of `{text, href}` items that renders as a `Related: link, link` line inside the dropdown body (slots into `order` as `related`, otherwise appends after the stack)
...
- After each real piece of progress or decision while working through `.Codex/tasks/improvement-ideas.md`, update that file (strike through the item with `~~...~~`, append `(done YYYY-MM-DD)`, and replace the body with a one-line note of what changed).
- Once an item is fully implemented, fold a short note about how it *currently* works into the relevant existing section of this AGENTS.md (e.g., extend a bullet under **Current Project State** or **Canonical Files**). General terms, not change history. Don't add new top-level sections just to track shipped work.
...
See `.Codex/tasks/improvement-ideas.md` for prioritized post-launch work.

Last Updated: 2026-07-19 (current portfolio project coverage)
